...
import { onListingSnapshots } from "~/lib/modules/SnapshotManaging/presentation/controllers/onListingSnapshots"
...
const onFileInputChanged = async ({ target }: Event & { target: HTMLInputElement }) => {
    ...
    const succeeded = await onUploadingSnapshot(tenantId, file.name, content)
    target.files = null
