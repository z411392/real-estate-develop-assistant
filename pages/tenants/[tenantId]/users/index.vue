...
import { onListingUsers } from "~/lib/modules/IdentityAndAccessManaging/presentation/controllers/onListingUsers"
...
const onReviewingButtonClicked = async (permissionId: string, status: PermissionStatuses) => {
    ...
    if (index < 0) return
    const originalStatus = items.value[index].permission.status
    items.value[index].permission.status = PermissionStatuses.Reviewing
    await queue.add(async () => {
        const succeeded = await onReviewingJoining(tenantId, permissionId, status)
        if (!succeeded) {
            items.value[index].permission.status = originalStatus
            return
        }
        const permission = await onRetrievingPermission(tenantId, permissionId)
        items.value[index].permission = permission
    })
}
</script>
